...
def make_open3d_corr_lines(ref_corr_points, src_corr_points, label):
    ...
    colors = np.zeros((num_correspondences, 3))
    for i in range(num_correspondences):
        if label[i] == True:
            colors[i] = [0.0, 1.0, 0.0]  # 设置为绿色
        elif label[i] == False:
            colors[i] = [1.0, 0.0, 0.0]  # 设置为红色
        else:
            raise ValueError("Unsupported `label` {} for correspondences".format(label[i]))
